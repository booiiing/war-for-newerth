    ...
    buildTable: function() {
        if(!this.tableShown) {
            ...
            for(i = 0; i < this.colourArray.length; i++) {
                ...
                html += "<td style=\"background-color: #" + this.colourArray[i] + ";\" title=\"#" + this.colourArray[i] +  "\" onClick=\"$('" + this.element.id + "').value = '#" + this.colourArray[i] + "'; $('" + this.trigger.id + "ColourPicker').style.display = 'none';$('" + this.element.id + "').style.backgroundColor='#" + this.colourArray[i] + "';\">";
                if(i % 8 == 7) {
                    html += "</tr>";
                }
            }
